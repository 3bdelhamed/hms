import { expect, type Locator, type Page } from '@playwright/test';

export interface FloorData {
  branch: string;
  building: string;
  code: string;
  floorNumber: number;
  nameAr: string;
  nameEn: string;
  description?: string;
}

export class FloorsPage {
  private readonly page: Page;
  private readonly searchBox: Locator;
  private readonly searchButton: Locator;
  private readonly addFloorLink: Locator;
  private readonly rows: Locator;
  private readonly saveButton: Locator;
  private readonly codeField: Locator;
  private readonly floorNumberField: Locator;
  private readonly nameArField: Locator;
  private readonly nameEnField: Locator;
  private readonly descriptionField: Locator;

  constructor(page: Page) {
    this.page = page;
    this.searchBox = page.getByPlaceholder('بحث باسم الطابق…');
    this.searchButton = page.getByRole('button', { name: 'بحث', exact: true });
    this.addFloorLink = page.getByRole('link', { name: 'إضافة طابق جديد' });
    this.rows = page.locator('tbody tr');
    this.saveButton = page.getByRole('button', { name: 'حفظ', exact: true });
    this.codeField = page.locator('#floor-code');
    this.floorNumberField = page.locator('#floor-number');
    this.nameArField = page.locator('#floor-name-ar');
    this.nameEnField = page.locator('#floor-name-en');
    this.descriptionField = page.locator('#floor-description');
  }

  async goto() {
    await this.page.goto('/structure/floors/explorer');
  }

  async searchFloor(query: string) {
    await this.searchBox.fill(query);
    await this.searchButton.click();
  }

  async openCreateFloor() {
    await this.addFloorLink.click();
  }

  // Selects an ng-select option and verifies the selection is applied.
  // Types into the dropdown search first so the click targets a stable,
  // filtered option (option lists re-render while lookups resolve).
  // If lookupAfterClick is given, waits for the cascade lookup it triggers.
  private async chooseOption(inputId: string, value: string, lookupAfterClick?: RegExp) {
    const panel = this.page.locator(`ng-select[inputid="${inputId}"]`);
    const searchBox = panel.locator('input');
    const matches = this.page.locator('.ng-option', { hasText: value });
    const applied = panel.locator('.ng-value-label', { hasText: value });
    const reloaded = lookupAfterClick
      ? this.page.waitForResponse(lookupAfterClick, { timeout: 30000 })
      : null;
    await panel.click();
    await searchBox.fill(value);
    // Settle gate: proceed only when the filtered list holds exactly our match.
    await expect(matches).toHaveCount(1, { timeout: 15000 });
    await matches.first().click();
    await reloaded;
    await this.page.keyboard.press('Escape');
    try {
      await expect(applied).toBeVisible({ timeout: 10000 });
    } catch {
      // Single recovery attempt: re-select once, then fail loud if still unapplied.
      await panel.click();
      await searchBox.fill(value);
      await expect(matches).toHaveCount(1, { timeout: 15000 });
      await matches.first().click();
      await this.page.keyboard.press('Escape');
      await expect(applied).toBeVisible({ timeout: 10000 });
    }
  }

  private async selectOption(inputId: string, value: string, lookupAfterClick?: RegExp) {
    await this.chooseOption(inputId, value, lookupAfterClick);
  }

  // Child select whose options depend on an already-selected parent.
  private async selectChildOption(inputId: string, value: string) {
    await this.chooseOption(inputId, value);
  }

  async createFloor(data: FloorData) {
    await this.selectOption('floor-branch-filter', data.branch, /\/api\/building\/lookup/);
    await this.selectChildOption('floor-building', data.building);
    await this.codeField.fill(data.code);
    await this.floorNumberField.fill(String(data.floorNumber));
    await this.nameArField.fill(data.nameAr);
    await this.nameEnField.fill(data.nameEn);
    if (data.description !== undefined) await this.descriptionField.fill(data.description);
    await this.saveButton.click();
    // App navigates to the new record's edit page only after the save completes.
    await this.page.waitForURL(/\/edit$/, { timeout: 20000 });
  }

  private row(name: string): Locator {
    return this.rows.filter({ hasText: name });
  }

  async expectFloorInList(name: string) {
    await expect(this.row(name)).toBeVisible();
  }

  async editFloor(name: string) {
    await this.row(name).getByRole('button', { name: 'تعديل' }).click();
  }

  async deactivateFloor(name: string) {
    await this.row(name).getByRole('button', { name: 'إلغاء التفعيل' }).click();
  }

  async deleteFloor(name: string) {
    await this.row(name).getByRole('button', { name: 'حذف' }).click();
  }
}
